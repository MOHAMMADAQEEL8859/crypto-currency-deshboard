...
//function to fetch the data
  ...
  return (
    <div className="transition ease-in-out mx-7 hover:ring-4 ring-black/60 relative lg:w-11/12 bg-gray-300 h-12 lg:h-14 flex items-center px-6 shadow-sm  rounded-md outline outline-3 outline-black/60">
      ...
      <input
        ...
        type="text"
        className=" placeholder:text-black text-base lg:text-lg text-black+
         font-medium h-full focus:outline-none w-full pl-2 ml-1 bg-gray-300"
        placeholder="You can search coins here"
      />
      {isOpen && (
        <div className="absolute w-full top-11 max-sm:top-9 right-0 z-10 mt-4 origin-top-right rounded-md border border-black bg-white shadow-lg">
          <div>
            ...
            {searchValues.length === 0 && (
              <div className="block rounded-lg px-4 py-2 max-sm:text-xs text-base text-black no-underline cursor-pointer ">
                No Result
              </div>
            )}
          </div>
