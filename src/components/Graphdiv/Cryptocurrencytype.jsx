...
const CryptocurrencySelector = () => {
  ...
  return (
    <div className="max-sm:w-10/12 w-5/12 max-lg:w-6/12 max-sm:h-6 h-10 flex rounded-md border-black border-2 hover:ring-2 ring-black ">
      <div className="w-full h-full items-center relative flex rounded-md bg-white text-black shadow-sm">
        ...
        <div onClick={toggling} className="h-full relative">
          <button type="button" className="flex h-full items-center justify-center rounded-r-md border-1  px-2 hover:bg-white">
        ...
        {isOpen && (
          <div className="min-w-[200px] max-h-[250px] absolute max-sm:top-2 top-7 right-0 z-10 mt-4 origin-top-right rounded-md border  bg-white shadow-lg overflow-y-scroll">
            <div>
              {cryptoCurrencies.map((currency, i) => (
                <div key={i} onClick={() => selectCurrency(currency.name)} className="flex rounded-lg items-center hover:ring-2 ring-black cursor-pointer gap-1 px-4 py-2 max-sm:text-xs text-base text-black no-underline">
                  {currency.name}
                </div>
              ))}
            </div>
